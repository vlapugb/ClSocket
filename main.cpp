...
void upload_segment(CActiveSocket &SocketActive, int32_t seg_num, bool need_confirm, vector<vector<int32_t>> nodes)
{
    ...
    ss_for_nodes << hex << nodes[i][0];
        string hexString = ss_for_nodes.str();
        string part_first = hexString.substr(0,1);
        string part_second = hexString.substr(1);
    buf[counter++] = uint8(stoi(part_second, nullptr, 16));
    buf[counter++] = uint8(stoi(part_first, nullptr, 16));
    }
    for(int j = first_node_idx; j <last_node_idx+1; j++)
    {
        stringstream ss_for_nodes;
        ss_for_nodes << hex << nodes[j][1];
        string hexString = ss_for_nodes.str();
        string part_first = hexString.substr(0,1);
        string part_second = hexString.substr(1);
    buf[counter++] = uint8(stoi(part_second, nullptr, 16));
...
void upload_traj(CActiveSocket& SocketActive, vector<vector<int32_t>> nodes){
    ...
    {
    case 1:
        buf[7] = uint8(stoi(hexString, nullptr, 16));
        break;
    case 2:
        buf[7] = uint8(stoi(hexString, nullptr, 16));
        break;
    case 3:
        ...
        buf[6] = uint8(stoi(part2, nullptr, 16));
        buf[7] = uint8(stoi(part1, nullptr, 16));
        break;
        }
    case 4:
        {
        ...
        buf[6] = uint8(stoi(part22, nullptr, 16));
        buf[7] = uint8(stoi(part11, nullptr, 16));
        }
    break;
    }
    ...
    {
    case 1:
        buf[5] = uint8(stoi(hexString2, nullptr, 16));
        break;
    case 2:
        buf[5] = uint8(stoi(hexString2, nullptr, 16));
        break;
    case 3:
        {
        string hexString2 = ss2.str();
        string part3 = hexString2.substr(0,1);
        string part4 = hexString2.substr(1);
        buf[4] = uint8(stoi(part4, nullptr, 16));
        buf[5] = uint8(stoi(part3, nullptr, 16));
        break;
        }
    case 4:
        {
        string hexString2 = ss2.str();
        string part33 = hexString2.substr(0,2);
        string part44 = hexString2.substr(2);
        buf[4] = uint8(stoi(part44, nullptr, 16));
        buf[5] = uint8(stoi(part33, nullptr, 16));
        break;
        }
    }
